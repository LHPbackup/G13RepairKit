...
![](./G13RepairKit_images/picture0.jpg)  

rev1.5より採用しているRKJXV122400Rは、素の状態でレバーを限界まで倒すとレバーロックされる仕様ですが、本キットをG13に組みつけると可動域が制限され、問題なくセンタリング動作します。  

G13はスティックカバーの軸がボディに当たることで可動域が制限されますが、ロックがかかる角度までかなりマージンがあります。（当方のテストでは軸が2mm以上摩耗して可動域が増えても正常動作することを確認しております。軸がそこまで摩耗することはないと思いますが…）  
...
![](./G13RepairKit_images/picture4c.jpg)
...
![](./G13RepairKit_images/picture4d.jpg)

スティックカバー裏面のZシーム（凹凸のモヤモヤが出ている）側が長円の長手方向です。こちらが左右になる角度に穴と軸の向きを合わせて差し込んで下さい。

ジョイスティックカバーは全商品アナログスティックに一度取り付けてフィッティングに問題ないこと確認したうえで発送しております。
...
・素の状態でレバーを限界まで倒すとロックされますが、問題ありません。  
G13に組み付ければ可動域が制限されるので正しくセンタリング動作します。  
